  ...
  module Auth
    module ExternalProvider
      ...
      def handle_disconnect(provider)
         Authentication.find_by_provider_and_user_id(provider.to_s, current_user.id).delete
         Rails.logger.debug "Successfully disconnected #{current_user.name} from #{provider.capitalize}"
         redirect_to users_path, flash: { notice: t('external_login.notices.disconnect', { user_name: current_user.name, provider: provider })}
      end
      ...
      def check_error_conditions(authentication, provider, omniauth)
         ...
         if authentication.nil?
           ...
           error_message = t('external_login.errors.account_associated_with_another_user', { provider: provider.capitalize} )
         end
         error_message
      end
      ...
      def get_user(omniauth, authentication)
         ...
         if connect_existing_user?
            ...
            if user.nil?
              ...
              Rails.logger.debug "Returning existing user in the system [id: #{user.id}] having email: #{user_email}"
            end
         end
         user
      end
